...
function projectRootProbePath(workspaceRoot) {
    ...
    if (activePath && (0, paths_1.containsPath)(workspaceRoot, activePath) && !(0, paths_1.isIgnoredWorkspacePath)(activePath)) {
        return activePath;
    }
    const vueDocument = nova.workspace.textDocuments.find((document) => document.syntax === "vue" &&
        typeof document.path === "string" &&
        (0, paths_1.containsPath)(workspaceRoot, document.path) &&
        !(0, paths_1.isIgnoredWorkspacePath)(document.path));
...
function logWorkspaceDebug(config, reason) {
    ...
    const signals = [
        `package.json=${(0, paths_1.exists)((0, paths_1.joinPath)(root, "package.json")) ? "yes" : "no"}`,
        `tsconfig=${(0, paths_1.exists)((0, paths_1.joinPath)(root, "tsconfig.json")) ? "yes" : "no"}`,
        `jsconfig=${(0, paths_1.exists)((0, paths_1.joinPath)(root, "jsconfig.json")) ? "yes" : "no"}`,
        `typescript=${(0, paths_1.exists)((0, paths_1.joinPath)(root, "node_modules", "typescript", "package.json")) ? "yes" : "no"}`,
        `vue=${(0, paths_1.exists)((0, paths_1.joinPath)(root, "node_modules", "vue", "package.json")) ? "yes" : "no"}`
    ];
    ...
    if (probePath) {
        const projectRoot = (0, paths_1.nearestProjectRoot)(probePath, root);
        if (projectRoot) {
            const projectSignals = [
                `package.json=${(0, paths_1.exists)((0, paths_1.joinPath)(projectRoot, "package.json")) ? "yes" : "no"}`,
                `tsconfig=${(0, paths_1.exists)((0, paths_1.joinPath)(projectRoot, "tsconfig.json")) ? "yes" : "no"}`,
                `jsconfig=${(0, paths_1.exists)((0, paths_1.joinPath)(projectRoot, "jsconfig.json")) ? "yes" : "no"}`,
                `typescript=${(0, paths_1.exists)((0, paths_1.joinPath)(projectRoot, "node_modules", "typescript", "package.json")) ? "yes" : "no"}`,
                `vue=${(0, paths_1.exists)((0, paths_1.joinPath)(projectRoot, "node_modules", "vue", "package.json")) ? "yes" : "no"}`
            ];
...
function logEditorDebug(config, editor, reason) {
    ...
    const flags = [
        `syntax=${syntax}`,
        `inside-workspace=${(0, paths_1.workspaceContains)(document.path)}`,
...
function registerWorkspaceDebugLogging(configProvider) {
    const getConfig = () => configProvider() ?? (0, index_1.readConfig)();
    const disposables = [];
    disposables.push(nova.workspace.onDidOpenTextDocument((document) => {
        ...
        (0, logger_1.debug)(config, `document opened: ${document.path || document.uri || "untitled"} (syntax=${document.syntax || "none"}, inside-workspace=${(0, paths_1.workspaceContains)(document.path)})`);
    }));
    disposables.push(nova.workspace.onDidAddTextEditor((editor) => {
        const config = getConfig();
        logEditorDebug(config, editor, `editor added: ${(0, paths_1.basename)(documentLabel(editor.document))}`);
